...
- 为后续样章基准、Provider adapter 选型和 ADR 提供候选池；
- 避免把匿名说话人标签误当成真实角色身份。
...
- 受 `docs/adr/0001-model-capabilities-via-provider-apis.md` 约束，VoxWeaver 不内置或托管任何模型；
...
4. **用户确认选型**：确认首批 Provider、备选 Provider 和明确不采用项，再决定是否更新正式计划或新增 ADR。
