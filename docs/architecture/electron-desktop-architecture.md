...
本节状态：`accepted_for_planning`。本节的 Gate 和决策记录可作为派发输入；本文其余架构内容仍保持文件头标记的 `draft`，不得覆盖上位范围、accepted ADR 或 schema。

本节是 ARCH 决策、状态和关闭证据的权威记录，不替代 M1～M8 各自的 Gate。项目负责人于 2026-08-08 冻结本节三个 Gate 的已选策略；精确执行输入未齐，因此当前均保持 `open`。

本节执行规则：

- 每个 Gate 必须保留状态、已选策略、直接阻塞项、关闭证据和打开时的唯一派发白名单；“阻塞”列表的补集不自动构成可执行范围。
- 策略确认只冻结实施分支，不补全外部事实、版本、阈值、runtime profile 或验收证据；任何缺失值都不得由执行模型猜测。
- Gate 仅在本节写入全部关闭材料后才能改为 `closed`：确认策略、决定人、日期和全部必填值齐全；要求 ADR 时 ADR 必须为 `accepted`；真实路径还必须具备授权夹具、无密钥命令、成功/失败判定、证据路径、执行环境和负责人。
- ARCH Gate 只能通过明确列出的里程碑 Gate、任务卡或验收状态施加限制。尚未拆成任务卡的能力即使 Gate 关闭也不能实施，必须先新增文件级白名单的原子任务卡。
- 一个 ARCH 分支被阻塞时，其他里程碑仍按各自 Gate 和 DAG 继续；全部可运行分支耗尽后才报告整体 `blocked`。

### ARCH-D01：首个正式支持平台

状态：`open`。

本地决策项 ARCH-C01 的候选：A）只选当前可完成完整打包、签名和交互回归的一组 OS/CPU（已选）；B）同时支持 macOS arm64 与 Windows x64；C）仅生成开发构建，暂不声明正式支持平台。

决策记录：方案 A；决定人为项目负责人；日期为 2026-08-08。
...
关闭证据：在本节固定目标 OS、最低版本、CPU、安装包格式、验证设备/runtime profile ID、负责人、可直接复制的打包/安装/启动/升级/回退命令、成功与失败判定以及证据路径。精确值未齐时保持 `open`。

### ARCH-D02：Electron 打包、签名与更新工具链

状态：`open`。

本地决策项 ARCH-C02 的候选：A）先冻结单一打包工具，签名和自动更新分别在取得证书/发布主体后启用（已选）；B）首版同时完成打包、签名、自动更新和回滚；C）只保留开发运行，不生成安装包。
...
本地决策项 ARCH-C03 的候选：A）按项目设置软配额和全局磁盘保护线，只清理无引用缓存/临时文件，正式 revision 需显式归档或删除确认（已选）；B）固定保留最近 N 个 revision，自动删除更早版本；C）不设配额且永不清理。
...
关闭证据：在本节固定项目软配额、全局保护线、可清理类别、保留期、恢复窗口、版本化 policy ID、临界值/并发/中断规则、预览和人工确认边界、恢复失败行为、测试夹具/hash、精确验证命令与证据路径。关闭 Gate 也不授权实现；必须先新增原子任务卡。

其余策略已确认但执行输入未齐的能力 Gate 由对应里程碑规格管理：

- [M2-D03：LLM Provider 契约、dialect 与数据策略](../milestones/M2.文本AI处理执行规格.md)；
- [M4-D04～D05：TTS Provider 与请求/恢复策略](../milestones/M4.语音生成执行规格.md)；
- [M5-D01～D02：ASR/VAD/说话人分析 Provider 与非模型音频检测边界](../milestones/M5.音频QA执行规格.md)；
- [M6-D01：章节音频处理方式](../milestones/M6.章节合成执行规格.md)。
